...
# load sense a data
sense_a_data = load_intensity_file('/u/home/s/shihuwen/project/wqe/affy100k_intensity/affy100k_xba_sense_a_intensity.txt')
sense_a_int = sense_a_data[0]
sense_a_snps = sense_a_data[1]
sense_a_inds = set(sense_a_data[2])

# load sense b data
sense_b_data = load_intensity_file('/u/home/s/shihuwen/project/wqe/affy100k_intensity/affy100k_xba_sense_b_intensity.txt')
...
# load low maf snps
problem_snps = set()
f = open('selected_snps.txt', 'r')
for line in f:
    line = line.strip()
    problem_snps.add(line)
f.close()

# generate plots
count = 0
for snp in in_common_snps:
    if(snp not in problem_snps):
        continue
