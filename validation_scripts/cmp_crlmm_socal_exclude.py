...
parser.add_option("-e", "--exclusion", dest="excfile")
...
excfile_nm = options.excfile

# check command line
if(crlmmfile_nm == None or socalfile_nm == None
   or snpsfile_nm == None or excfile_nm == None):
    ...
    sys.stderr.write("\tUse -e to specify excfile\n")
    sys.exit()

# load in exc file
snp_exc = dict()
excfile = open(excfile_nm, 'r')
for line in excfile:
    line = line.strip()
    cols = line.split('\t')
    rsid = cols[0]
    excindv = set(cols[1].split(','))
    snp_exc[rsid] = excindv
...
for line in crlmmfile:
    ...
    # get individuals to exclude
    excindv = snp_exc[snpid]
    ...
    for indv in socal_indv_geno:
        if(indv not in crlmm_indv_geno or indv in excindv):
            continue
...
print 'correct call %d: ', naaaa+nabab+nbbbb
print 'total call %d: ', tot1+tot2+tot3
