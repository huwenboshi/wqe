...
And reference genotype calls were obtained using HapMart.
...
and those available on HapMap through leave-one-out cross-validation.
For each SNP, one sample was used as validation data and the rest were used
as training data.
The process was repeated until every sample were used as validation data
exactly once.

\par
Table~\ref{table:cmp_hapmap_socal} shows the genotype calls made by SoCal
