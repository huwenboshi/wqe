...
In SoCal, $\beta_1$,$\beta_2$,$\beta_3$,$\beta_4$ are empirically set to 1,10,
100, and 30 respectively.

\par
Similary, to find the ellipsoid that includes $X_{AB}$ and excludes $X_{AA}$
and $X_{BB}$, one sets $\{a_i\}=X_{AB}$ and $\{b_j\}=X_{AA} \cup X_{BB}$, and
solves the above conic programming problem.
The same procedure also applies to finding the ellipsoid that includes $X_{BB}$
and excludes $X_{AA}$ and $X_{AB}$.

% handling missing clusters
\subsection{Handling missing genotype clusters}
...
If a SNP has low MAF, one genotype cluster may be missing 
...
For SNPs with two missing genotype clusters, SoCal will not attempt to find
any decision region.
