...
\title{SoCal: supervised genotype calling via ellipsoidal
separation for Affymetrix SNP microarray}

\author{Huwenbo Shi (603-778-363) shihuwenbo@ucla.edu}
...
methods to identify these clusters and then assign genotypes to each cluster.
...
Gaussian decision regions for each genotype.
...
\begin{figure}[H]
\centering
\includegraphics[scale=0.5]{intro_figs/intro_genotype_clusters.pdf}
...
of the three genotypes of a SNP by solving a conic programming problem.
SoCal can control the effect of outliers by assigning different weights to each
of the criteria of finding the ellipsoids---separation ratio, ellipsoid
volume, and number of enclosed points. 
After the ellipsoidal decision regions are obtained, they are used to call
genotypes for the same SNP in linear time.
%-----------------------------------------------------------------------------
