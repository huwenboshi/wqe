...
In Affymetrix SNP microarrays, oligonucleotide probes are used to match and
...
Although these methods are generally applicable to a wide range of SNPs and

To utilize reference genotype calls, Rabbee et al. proposed the RLMM algorithm,
a supervised genotype calling method that uses reference genotype calls to form decision regions for each
genotype.
These decision regions are then used to call SNPs with unknown genotype.
All of the methods above assume that the log allele intensities generated
by SNPs follow bivariate Gaussian distributions.
