...
intensities of samples with reference genotype calls, SoCal uses these data
to efficiently finds ellipsoidal decision regions for each genotype cluster
via ellipsoidal separation by solving a conic programming problem.
...
($\beta_i$) assigned to the criteria of finding the separating ellipsoids
for different microarrays.
However, experiments with SoCal show that it's not sensitive to weight
parameters if $\beta_3$ is set to $10^2$.
...
decision regions.
These refined decision regions can then be also used to detect outliers and
...
limitation is also present in other supervised genotype callers, and has been
addressed.
